...
async def search_film_full(film_info: FilmSearch, current_user: User =  Depends(get_current_active_user)):
    if not await user_crud.is_active(current_user):
        raise HTTPException(status_code = 400, detail="profile is not active")
...
@router.get("/history-search", response_model = list[FilmSearch], status_code = status.HTTP_200_OK)
